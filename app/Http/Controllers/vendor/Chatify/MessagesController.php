...
class MessagesController extends Controller
{
    ...
    {
        $users = Message::join('youths',  function ($join) {
            $join->on('ch_messages.from_id', '=', 'youths.id')
                ->orOn('ch_messages.to_id', '=', 'youths.id');
        })
            ->where(function ($q) {
                $q->where('ch_messages.from_id', Auth::user()->id)
                    ->orWhere('ch_messages.to_id', Auth::user()->id);
            })
            ->where('youths.id','!=',Auth::user()->id)
            ->select('youths.*',DB::raw('MAX(ch_messages.created_at) as max_created_at'))
            ->groupBy('youths.id') // Add this line
    ...
    public function search(Request $request)
    {
        ...
        $records = Youth::where('id','!=',Auth::user()->id)
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? Youth::where('id', Auth::user()->id)->update(['dark_mode' => 1])  // Make Dark
                : Youth::where('id', Auth::user()->id)->update(['dark_mode' => 0]); // Make Light
        }

        // If messenger color selected
        if ($request['messengerColor']) {
            $messenger_color = trim(filter_var($request['messengerColor']));
            Youth::where('id', Auth::user()->id)
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->extension()), $allowed_images)) {
                    ...
                    $update = Youth::where('id', Auth::user()->id)->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        ...
        $status = Youth::where('id', Auth::user()->id)->update(['active_status' => $activeStatus]);
        return Response::json([
            'status' => $status,
        ], 200);
    }
